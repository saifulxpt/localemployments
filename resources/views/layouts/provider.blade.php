...
<body class="bg-gray-50 text-gray-900 font-sans antialiased" x-data="{ mobileMenuOpen: false }">
    ...
    {{-- Mobile Bottom Navigation Bar (5-Tab App Bar) --}}
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-4px_25px_rgba(0,0,0,0.08)] z-50 flex justify-around items-center h-16 px-1 pb-[env(safe-area-inset-bottom)]">
        ...
        {{-- 2. Browse Jobs --}}
        <a href="{{ route('provider.jobs.index') }}" class="flex flex-col items-center justify-center flex-1 h-full py-1 {{ request()->routeIs('provider.jobs.*') ? 'text-primary-600 font-bold' : 'text-gray-500 hover:text-primary-600' }} transition-colors">
            <svg class="w-5 h-5 mb-1" fill="{{ request()->routeIs('provider.jobs.*') ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor" stroke-width="{{ request()->routeIs('provider.jobs.*') ? '0' : '2' }}">
                <path stroke-linecap="round" stroke-linejoin="round" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
            </svg>
            <span class="text-[10px] tracking-tight">কাজ খুঁজুন</span>
        </a>

        {{-- 3. Center Elevated Bids Button --}}
        <div class="relative -top-4 flex flex-col items-center justify-center flex-1">
            <a href="{{ route('provider.bids.index') }}" class="w-12 h-12 bg-primary-700 hover:bg-primary-800 text-white rounded-full flex items-center justify-center shadow-lg shadow-primary-900/30 border-2 border-white ring-2 ring-primary-100 transform active:scale-95 hover:scale-105 transition-all">
                <svg class="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
            </a>
            <span class="text-[10px] font-bold text-primary-800 mt-0.5">বিডসমূহ</span>
        </div>

        {{-- 4. Bookings --}}
        <a href="{{ route('provider.bookings.index') }}" class="flex flex-col items-center justify-center flex-1 h-full py-1 {{ request()->routeIs('provider.bookings.*') ? 'text-primary-600 font-bold' : 'text-gray-500 hover:text-primary-600' }} transition-colors">
            <svg class="w-5 h-5 mb-1" fill="{{ request()->routeIs('provider.bookings.*') ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor" stroke-width="{{ request()->routeIs('provider.bookings.*') ? '0' : '2' }}">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>
            </svg>
            <span class="text-[10px] tracking-tight">বুকিং</span>
        </a>
