...
if (!function_exists('category_icon')) {
    ...
    {
        if (empty($name)) {
            return '<svg class="'.$classes.'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M5 13l4 4L19 7"/></svg>';
        }

        if (str_starts_with(trim($name), '<svg')) {
            return $name;
        }
