    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" 
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4 space-y-1.5 custom-scrollbar">
                ...
                <a href="{{ route('admin.settings.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium {{ active_class('admin.settings.index', $activeNav, $inactiveNav) }}">
                    <svg class="w-5 h-5 {{ active_class('admin.settings.index', 'text-white', 'text-gray-400') }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/></svg>
                    Settings
                </a>

                <a href="{{ route('admin.settings.api') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium {{ active_class('admin.settings.api', $activeNav, $inactiveNav) }}">
                    <svg class="w-5 h-5 {{ active_class('admin.settings.api', 'text-white', 'text-gray-400') }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                    API Settings
                </a>
