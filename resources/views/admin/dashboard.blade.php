...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">প্লাটফর্মের সার্বিক অবস্থার সারাংশ।</p>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div class="bg-blue-600 rounded-2xl p-6 shadow-sm border border-blue-700 text-white flex flex-col justify-between">
            <div class="flex justify-between items-start mb-2">
                <div class="text-blue-100 text-sm font-medium">মোট ইউজার</div>
            ...
            <div class="text-xs text-blue-200 mt-2 flex gap-3">
                <span>{{ number_format($stats['total_providers']) }} প্রোভাইডার</span>
                <span>{{ number_format($stats['total_seekers']) }} সিকার</span>
            </div>
        </div>
        
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex flex-col justify-between">
            <div class="flex justify-between items-start mb-2">
                <div class="text-gray-500 text-sm font-medium">সম্পন্ন বুকিং</div>
            ...
            <div class="text-xs text-gray-400 mt-2">মোট বুকিং: {{ number_format($stats['total_bookings']) }}</div>
        </div>

        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex flex-col justify-between">
            <div class="flex justify-between items-start mb-2">
                <div class="text-gray-500 text-sm font-medium">প্লাটফর্ম আয় (Fee)</div>
            ...
            <div class="text-xs text-gray-400 mt-2">মোট লেনদেন: ৳{{ number_format($stats['total_revenue']) }}</div>
        </div>

        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-200 flex flex-col justify-between">
            <div class="flex justify-between items-start mb-2">
                <div class="text-gray-500 text-sm font-medium">উন্মুক্ত কাজ (Jobs)</div>
            ...
            <a href="{{ route('admin.job-requests.index') }}" class="text-xs font-semibold text-blue-600 hover:underline mt-2">সব কাজ দেখুন &rarr;</a>
        </div>
    </div>

    {{-- Attention Required Grid --}}
    <h2 class="text-lg font-bold text-gray-900 mt-8 mb-4">অ্যাকশন প্রয়োজন (Pending Actions)</h2>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-red-100 p-5 flex items-center justify-between {{ $stats['pending_withdrawals'] > 0 ? 'bg-red-50/30' : '' }}">
            <div>
                ...
                <p class="text-sm text-gray-500">পেন্ডিং উইথড্র রিকোয়েস্ট</p>
            </div>
            <a href="{{ route('admin.withdrawals.index', ['status' => 'pending']) }}" class="btn btn-outline border-gray-200 btn-sm">রিভিউ করুন</a>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-orange-100 p-5 flex items-center justify-between {{ $stats['open_disputes'] > 0 ? 'bg-orange-50/30' : '' }}">
            <div>
                ...
                <p class="text-sm text-gray-500">ওপেন ডিসপুট (Disputes)</p>
            </div>
            <a href="{{ route('admin.disputes.index', ['status' => 'open']) }}" class="btn btn-outline border-gray-200 btn-sm">রিভিউ করুন</a>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-blue-100 p-5 flex items-center justify-between {{ $stats['pending_verifications'] > 0 ? 'bg-blue-50/30' : '' }}">
            <div>
                ...
                <p class="text-sm text-gray-500">পেন্ডিং ভেরিফিকেশন</p>
            </div>
            <a href="{{ route('admin.verifications.index') }}" class="btn btn-outline border-gray-200 btn-sm">রিভিউ করুন</a>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 class="font-bold text-gray-900">সাম্প্রতিক বুকিং</h3>
                <a href="{{ route('admin.bookings.index') }}" class="text-sm font-semibold text-blue-600 hover:underline">সব দেখুন</a>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 class="font-bold text-gray-900">নতুন ইউজার</h3>
                <a href="{{ route('admin.users.index') }}" class="text-sm font-semibold text-blue-600 hover:underline">সব দেখুন</a>
            </div>
            <div class="divide-y divide-gray-100">
