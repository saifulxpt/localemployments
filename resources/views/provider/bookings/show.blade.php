                ...
                @elseif($booking->directService)
                    <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 text-sm space-y-2">
                        <div class="flex gap-2">
                            ...
                            <span class="text-gray-900">{{ $booking->jobRequest->description ?? 'কোনো নোট নেই' }}</span> {{-- Direct booking note is saved in jobRequest's description logically or somewhere else. Assuming in job request description or a specific field --}}
                        </div>
                    </div>
                @endif
            </div>
