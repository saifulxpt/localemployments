...
<body class="bg-gray-50 text-gray-900 font-sans antialiased" x-data="{ sidebarOpen: false }">
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-4px_25px_rgba(0,0,0,0.08)] z-50 flex justify-around items-center h-16 px-1 pb-[env(safe-area-inset-bottom)]">
        ...
        <div class="relative -top-4 flex flex-col items-center justify-center flex-1">
            <a href="{{ route('seeker.job-requests.create') }}" class="w-12 h-12 bg-primary-700 hover:bg-primary-800 text-white rounded-full flex items-center justify-center shadow-lg shadow-primary-900/30 border-2 border-white ring-2 ring-primary-100 transform active:scale-95 hover:scale-105 transition-all">
        ...
        {{-- 4. My Jobs --}}
        <a href="{{ route('seeker.job-requests.index') }}" class="flex flex-col items-center justify-center flex-1 h-full py-1 {{ request()->routeIs('seeker.job-requests.*') || request()->routeIs('seeker.dashboard') ? 'text-primary-600 font-bold' : 'text-gray-500 hover:text-primary-600' }} transition-colors">
            <svg class="w-5 h-5 mb-1" fill="{{ request()->routeIs('seeker.job-requests.*') || request()->routeIs('seeker.dashboard') ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor" stroke-width="{{ request()->routeIs('seeker.job-requests.*') || request()->routeIs('seeker.dashboard') ? '0' : '2' }}">
