...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">প্রোভাইডারদের সাথে আপনার কথোপকথন।</p>
    </div>
    ...
    @if($bookings->count() > 0)
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-100">
            @foreach($bookings as $booking)
                ...
                <a href="{{ route('seeker.messages.show', $booking->id) }}" class="flex items-start gap-4 p-4 md:p-6 hover:bg-gray-50 transition-colors {{ $hasUnread ? 'bg-primary-50/30' : '' }}">
                    <div class="relative shrink-0">
                        <img src="{{ $booking->provider->avatar_url }}" alt="" class="w-12 h-12 md:w-14 md:h-14 rounded-full object-cover border border-gray-200">
                        @if($hasUnread)
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-start mb-1">
                            <h3 class="font-bold text-gray-900 truncate {{ $hasUnread ? 'text-primary-900' : '' }}">{{ $booking->provider->name }}</h3>
                            @if($lastMessage)
