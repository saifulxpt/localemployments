...
<div class="max-w-6xl mx-auto space-y-6" x-data="{ activeTab: 'overview' }">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            @if($user->role === 'provider' && $user->providerProfile && !$user->providerProfile->is_verified)
                <form action="{{ route('admin.users.verify-profile', $user->id) }}" method="POST" onsubmit="return confirm('ম্যানুয়ালি প্রোফাইল ভেরিফাই করতে চান?')">
                    @csrf
                    <button type="submit" class="btn btn-outline border-green-200 text-green-600 hover:bg-green-50 btn-sm">প্রোফাইল ভেরিফাই করুন</button>
                </form>
            @endif
    ...
    <div class="border-b border-gray-200">
        <nav class="-mb-px flex space-x-6" aria-label="Tabs">
            ...
            <button @click="activeTab = 'activity'" 
                ...
                অ্যাক্টিভিটি লগ 
                <span class="bg-gray-100 text-gray-600 py-0.5 px-2 rounded-full text-xs">{{ $activities->count() }}</span>
            </button>
            ...
            <button @click="activeTab = 'sms'" 
                    ...
                    class="whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2">
                SMS হিস্ট্রি
                <span class="bg-gray-100 text-gray-600 py-0.5 px-2 rounded-full text-xs">{{ $smsLogs->count() }}</span>
            </button>
        </nav>
    </div>
    ...
    <div x-show="activeTab === 'activity'" style="display: none;" x-transition.opacity>
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6 border-b border-gray-100">
                <h3 class="font-bold text-gray-900">অ্যাডমিন অ্যাক্টিভিটি লগ</h3>
                <p class="text-sm text-gray-500">এই ইউজারের উপর অ্যাডমিনদের বিভিন্ন অ্যাকশন।</p>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 text-gray-600 text-sm border-b border-gray-100">
                                <th class="p-4 font-semibold">তারিখ ও সময়</th>
                                <th class="p-4 font-semibold">অ্যাডমিন</th>
                                <th class="p-4 font-semibold">অ্যাকশন</th>
                                <th class="p-4 font-semibold">আইপি (IP)</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 text-sm">
                            @foreach($activities as $log)
                                <tr class="hover:bg-gray-50">
                                    <td class="p-4 whitespace-nowrap text-gray-500">{{ $log->created_at->format('d M Y, h:i A') }}</td>
                                    <td class="p-4 font-medium text-gray-900">{{ $log->admin->name ?? 'Unknown' }}</td>
                                    <td class="p-4 text-gray-700">{{ $log->action }}</td>
                                    <td class="p-4 text-gray-500">{{ $log->ip_address }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
            ...
            @else
                <div class="p-12 text-center text-gray-500">কোনো অ্যাক্টিভিটি লগ পাওয়া যায়নি।</div>
            @endif
        </div>
    </div>

    {{-- TAB CONTENT: SMS HISTORY --}}
    <div x-show="activeTab === 'sms'" style="display: none;" x-transition.opacity>
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="p-6 border-b border-gray-100">
                <h3 class="font-bold text-gray-900">SMS হিস্ট্রি</h3>
                <p class="text-sm text-gray-500">এই ইউজারকে পাঠানো সমস্ত SMS এর তালিকা।</p>
            </div>
            @if($smsLogs->count() > 0)
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 text-gray-600 text-sm border-b border-gray-100">
                                <th class="p-4 font-semibold">তারিখ ও সময়</th>
                                <th class="p-4 font-semibold">টাইপ</th>
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            @foreach($smsLogs as $sms)
                                <tr class="hover:bg-gray-50">
                                    <td class="p-4 whitespace-nowrap text-gray-500 align-top">{{ $sms->created_at->format('d M Y, h:i A') }}</td>
                                    <td class="p-4 align-top">
                                        <span class="inline-block px-2 py-1 bg-gray-100 text-gray-700 rounded text-xs uppercase">{{ $sms->type }}</span>
                                    </td>
                                    <td class="p-4 text-gray-700 max-w-md whitespace-pre-wrap">{{ $sms->message }}</td>
                                    <td class="p-4 align-top">
                                        @if($sms->status === 'sent')
                                            <span class="inline-flex items-center gap-1 text-green-600 font-medium"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Sent</span>
                                        @else
                                            <span class="inline-flex items-center gap-1 text-red-600 font-medium"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg> Failed</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
            ...
            @else
                <div class="p-12 text-center text-gray-500">কোনো SMS লগ পাওয়া যায়নি।</div>
            @endif
        </div>
    </div>
