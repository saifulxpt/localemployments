...
@section('title', 'চ্যাট - ' . $booking->seeker->name)
...
<div class="max-w-4xl mx-auto h-[calc(100vh-140px)] flex flex-col bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" 
     x-data="chatComponent('{{ route('provider.messages.poll', $booking->id) }}', '{{ route('provider.messages.send', $booking->id) }}')"
     x-init="initChat()">

    {{-- Chat Header --}}
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-4">
            <a href="{{ route('provider.messages.index') }}" class="text-gray-500 hover:text-primary-600 transition-colors">
                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            </a>
            <div class="flex items-center gap-3">
                <img src="{{ $booking->seeker->avatar_url }}" alt="" class="w-10 h-10 rounded-full object-cover border border-gray-200">
                <div>
                    <h2 class="font-bold text-gray-900">{{ $booking->seeker->name }}</h2>
                    <p class="text-xs text-gray-500 font-medium">বুকিং #{{ $booking->id }} | কাস্টমার</p>
                </div>
            </div>
        </div>
        
        <a href="{{ route('provider.bookings.show', $booking->id) }}" class="btn btn-outline btn-sm hidden md:inline-flex">বুকিং বিস্তারিত</a>
    </div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('chatComponent', (pollUrl, sendUrl) => ({
            ...
            sendMessage() {
                ...
                // Optimistic UI update could go here, but simple approach is to wait for fetch
                ...
                .catch(() => {
                    this.sending = false;
                    this.newMessage = msg; // Restore on failure
